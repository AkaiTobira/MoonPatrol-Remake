...
onready var saved_player_position = player.position
func update_precision():
	if precision_update_timer < max_precision_update_timer: return
	precision_update_timer = 0 
	var from_left  = player.position.x > (saved_player_position.x - 10)
	var from_right = player.position.x < (saved_player_position.x + 10)
	if from_left and from_right: precision = max( 1, precision - 30 )
	else: precision = min( MAX_PRECISION, precision + 30 )
...
func select_dead_point():
	...
	if not get_parent().active_squats[squat_id][1]:
		if randi()%5 == 0: 
			...
			point_in_space_over_player = player.position + Vector2( randi()%20 * -1 if randi()%2 == 0 else 1, 10 ) 
	move_free = true
	dead      = true
...
func process_dead():
	...
	select_dead_point()
	if point_in_space_over_player.distance_to( position ) < 3: on_dead()
...
func spawn_missle():
	...
	if randi()%2 == 0:
		missle_target.x -= (precision - randi()% precision)
	else:
		missle_target.x += (precision - randi()% precision)
